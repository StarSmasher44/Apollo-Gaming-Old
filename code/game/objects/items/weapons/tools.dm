...
/obj/item/weapon/wirecutters/attack(mob/living/carbon/human/C as mob, mob/user as mob)
	if(C.handcuffed)
		if(C.species.name == "Wryn" && (locate(C.internal_organs_by_name["antennae"]) in C.internal_organs) && user.a_intent == I_HURT)
			...
			if(user.a_intent == I_HELP && (istype(C.handcuffed, /obj/item/weapon/handcuffs/cable)))
